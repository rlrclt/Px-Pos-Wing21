...
import { X, Link as LinkIcon, RefreshCw, AlertTriangle } from 'lucide-react';
...
export const LinkFolderModal: React.FC<LinkFolderModalProps> = ({
  isOpen,
  onClose,
  onFolderLinked
}) => {
  ...
  const [folderUrlOrId, setFolderUrlOrId] = useState('');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!folderUrlOrId.trim()) {
      setErrorMsg('กรุณาระบุ URL หรือ Folder ID ของ Google Drive');
      return;
    }
    ...
    const toastId = showToast({
      type: 'loading',
      title: 'กำลังเชื่อมโยงโฟลเดอร์ Google Drive...',
      message: 'ตรวจสอบการมีอยู่ของโฟลเดอร์บน Google Drive'
    }, 0);

    try {
      const res = await api.linkDriveFolder({
        folder_url_or_id: folderUrlOrId.trim(),
      ...
      if (res.success) {
        showToast({
          type: 'success',
          title: 'ผูกโฟลเดอร์สำเร็จ!',
          message: `เชื่อมต่อโฟลเดอร์ "${res.folder?.folder_name || 'Google Drive Folder'}" เรียบร้อย`
        });
        setFolderUrlOrId('');
        setFolderName('');
        onFolderLinked();
        ...
        setErrorMsg(res.error || 'ไม่สามารถผูกโฟลเดอร์ได้');
      }
    } catch (err: any) {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className={`text-base font-bold ${isLight ? 'text-slate-900' : 'text-white'}`}>
                ผูกโฟลเดอร์ Google Drive เดิม
              </h3>
              <p className={`text-xs mt-0.5 ${isLight ? 'text-slate-500' : 'text-slate-400'}`}>
                ระบุ URL โฟลเดอร์ที่แชร์หรือสร้างไว้แล้วใน Google Drive
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 text-xs">
          ...
          <div>
            <label className={`block font-semibold mb-1 ${isLight ? 'text-slate-700' : 'text-slate-300'}`}>
              ลิงก์โฟลเดอร์ Google Drive หรือ Folder ID <span className="text-rose-500">*</span>
            </label>
            <input
              type="text"
              required
              placeholder="https://drive.google.com/drive/folders/1rg4IQR..."
              value={folderUrlOrId}
              onChange={(e) => setFolderUrlOrId(e.target.value)}
              disabled={isSubmitting}
              className={`w-full px-3 py-2 rounded-xl border font-mono text-xs focus:outline-none transition-colors ${
                isLight 
          ...
          <div>
            <label className={`block font-semibold mb-1 ${isLight ? 'text-slate-700' : 'text-slate-300'}`}>
              ชื่อโฟลเดอร์ที่ต้องการแสดง (ไม่บังคับ)
            </label>
            <input
              type="text"
              placeholder="เว้นว่างไว้เพื่อดึงชื่อเดิมจาก Google Drive"
              value={folderName}
